...
| --- | --- | --- | --- |
| `request_credential_access` | Explicit user action only. Requests macOS permission for an existing credential source. Use `account_id` for a saved account or `profile_id` for a pending profile reported by discovery; the two fields are mutually exclusive. The account must belong to the provider and the profile must be configured and enabled. Omitting both is accepted only when there is no ambiguity between profiles. Never starts browser login, enables polling, or grants future background interaction. Clients should allow 65 seconds. | May display one system permission prompt per call; cancellation is not retried. | `unknown_provider`, `unknown_account`, `invalid_argument`, `storage_unavailable`, `internal` |
