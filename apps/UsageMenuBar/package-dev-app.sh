...
CARGO_TARGET_DIR="$repo_dir/target" \
  cargo build --release --manifest-path "$repo_dir/Cargo.toml" -p usage-daemon

swift_bin_dir="$(swift build -c release --package-path "$app_dir" --show-bin-path)"

rm -rf "$app"
mkdir -p "$app/Contents/MacOS" "$app/Contents/Resources"
cp "$app_dir/Info.plist" "$app/Contents/Info.plist"
cp "$swift_bin_dir/UsageMenuBar" "$app/Contents/MacOS/UsageMenuBar"
cp "$repo_dir/target/release/usage-daemon" "$app/Contents/MacOS/usage-daemon"
