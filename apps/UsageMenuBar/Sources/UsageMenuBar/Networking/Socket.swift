...
enum Socket {
    static func canConnect(path: String, timeout: Double) -> Bool {
        let fd = socket(AF_UNIX, SOCK_STREAM, 0)
        guard fd >= 0 else { return false }
        defer { close(fd) }
        do {
            try connect(fd: fd, path: path, deadline: Date().addingTimeInterval(timeout))
            return true
        } catch {
            return false
        }
    ...
    static func line(path: String, request: String, timeout: Double) throws -> String {
        ...
        try connect(fd: fd, path: path, deadline: deadline)
    ...
    private static func connect(fd: Int32, path: String, deadline: Date) throws {
