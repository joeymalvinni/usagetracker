...
use usage_core::{
    ...
    ProviderHealthStatus, ProviderId, UsageSnapshot, UsageWindowKind,
};
use uuid::Uuid;
...
const FORECAST_OBSERVATIONS_MIGRATION: &str =
    include_str!("../migrations/0008_forecast_observations.sql");
...
impl Storage {
    pub fn open(path: &Path) -> anyhow::Result<Self> {
        ...
        conn.execute_batch(FORECAST_OBSERVATIONS_MIGRATION)?;
        Ok(Self {
...
fn accounts_from_conn(conn: &Connection) -> anyhow::Result<Vec<Account>> {
    let mut stmt = conn.prepare(
...
mod tests {
    ...
    async fn usage_dashboard_reads_bounded_compact_forecast_history() {
        ...
        let (observation_count, duplicate_metadata_count, uses_covering_index) = storage
            .with_connection(|conn| {
                ...
                Ok((
                    ...
                    conn.query_row(
                        "SELECT COUNT(*) FROM usage_snapshots WHERE metadata_json IS NOT NULL",
                        [],
                        |row| row.get::<_, i64>(0),
                    )?,
        ...
        assert_eq!(duplicate_metadata_count, 0);
        assert!(uses_covering_index);
    }

    #[tokio::test]
    async fn forecast_observation_migration_backfills_once() {
        ...
        let account = storage
            ...
            .await
            .unwrap();
        let collected_at = Utc.with_ymd_and_hms(2026, 7, 10, 10, 0, 0).unwrap();
        storage
            .insert_snapshot(
                &UsageSnapshot {
                    provider_id,
                    ...
                    windows: vec![percentage_window(
                        ...
                        UsageWindowKind::Session,
                        25.0,
                        Some(collected_at + chrono::TimeDelta::hours(5)),
                    )],
                    metadata: json!({}),
                },
                None,
        ...
        storage
            .with_connection(|conn| {
                conn.execute("DELETE FROM usage_window_observations", [])?;
                conn.execute("DELETE FROM usage_schema_migrations WHERE version = 7", [])?;
                Ok(())
            })
            .await
            .unwrap();
        ...
        let storage = Storage::open(&path).unwrap();
        let (observations, markers) = storage
            .with_connection(|conn| {
                Ok((
                    conn.query_row(
                        "SELECT COUNT(*) FROM usage_window_observations",
                    ...
                    conn.query_row(
                        "SELECT COUNT(*) FROM usage_schema_migrations WHERE version = 7",
                        [],
                        |row| row.get::<_, i64>(0),
                    )?,
                ))
        ...
        assert_eq!(
            observations, 0,
            "completed backfill must not rescan at startup"
        );
        drop(storage);
